...
{
public:
  ImageGrabber(ORB_SLAM3::System *pSLAM, ImuGrabber *pImuGb, const bool bClahe, const string &paras) : mpSLAM(pSLAM), mpImuGb(pImuGb), mbClahe(bClahe)
  {
...
int main(int argc, char **argv)
{
  ...
  ImuGrabber imugb;
  ImageGrabber igb(&SLAM, &imugb, bEqual, argv[2]); // TODO
...
void ImageGrabber::SyncWithImu()
{
  ...
  while (1)
  {
    ...
    if (!img0Buf.empty() && !mpImuGb->imuBuf.empty())
    {
      tIm = img0Buf.front()->header.stamp.toSec() + mtd;

      if (tIm > mpImuGb->imuBuf.back()->header.stamp.toSec())
      {
        cout << "///////////"<<endl;
        cout << "tIm = " << tIm << endl;
        cout << "tImu =" << mpImuGb->imuBuf.back()->header.stamp.toSec() << endl;
        continue;
      }
